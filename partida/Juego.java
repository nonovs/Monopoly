...
public class Juego {
    ...
    // --- CORRECCIÓN CRÍTICA EN ESTE MÉTODO ---
    private void moverYEvaluar(Jugador j, int pasos) {
        int posIni = j.getPosicion();
        int posFin = (posIni + pasos) % 40;

        // Detectar si pasamos por la salida
        if (posIni + pasos >= 40) {
            j.sumarFortuna((float) Valor.SUMA_VUELTA);
            j.acumularPasarPorSalida((float) Valor.SUMA_VUELTA);

            // CORRECCIÓN: Solo sumamos vuelta aquí si NO caemos en la Salida (pos 0).
            // Si caemos en pos 0, la propia Casilla Salida sumará la vuelta al evaluarse.
            if (posFin != 0) {
                j.setVueltas();
            }
    ...
    // --- ESTADÍSTICAS GLOBALES CORREGIDAS (FORMATO NUMÉRICO) ---
    public void mostrarEstadisticasJuego() {
        ...
        // Corregido el formato a %.0f para evitar notación científica
